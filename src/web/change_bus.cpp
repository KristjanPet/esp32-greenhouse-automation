...
void updateStatusCache(bool manual, const char* motorStateStr) {
  if (manual != lastManual || lastMotorState != motorStateStr) {
    ...
    statusJson = "{";
    statusJson += "\"manual\":"; statusJson += (manual ? "true" : "false"); statusJson += ",";
    statusJson += "\"motorState\":\""; statusJson += motorStateStr; statusJson += "\",";
    statusJson += "\"v\":"; statusJson += String(++statusV);
    statusJson += "}";
  }
}
